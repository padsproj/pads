...
# $Id: copybook.py,v 1.1 2003/05/08 17:23:29 gruber Exp $
...
def unravel(idm, rep):
   ...
   if 'pic' not in m:
      return rp + '%02d %s %s none' % (m['level'], mapped_name, sz)

   # Otherwise, there is a picture
   return rp + '%02d %s %s %s' % (m['level'], mapped_name, sz, decode_pic(m))
...
print >>sys.stderr, "copybook.py", "$Revision: 1.1 $".split()[1], \
      "$Date: 2003/05/08 17:23:29 $".split()[1]
