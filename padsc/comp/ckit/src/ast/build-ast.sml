...
struct
  ...
  fun isZeroExp(Ast.EXPR (Ast.IntConst i, _, _)) = i = IntInf.fromInt 0 
    | isZeroExp _ = false

  fun isZeroCoreExp(Ast.IntConst i) = i = IntInf.fromInt 0
...
let
 ...
 (* miscellaneous utility functions *)
  ...
  fun ItoWord (i:IntInf.int) = W.fromLargeInt (IntInf.toLarge i)
      handle OverFlow => (error ("Bit operations on "^
				 "too large integer.\n"); 
			  W.fromInt 0)
  fun IfromWord(w:W.word) = IntInf.fromLarge(W.toLargeInt w)
...
*)
  ...
  and cnvExpression expr = 
	...
	let 
	  val (oneTy, one) = wrapEXPR(stdInt, Ast.IntConst (IntInf.fromInt 1))  (* implicit one constant
...
(**** old code: delete in due course 
  ...
  and evalExpr (e) : IntInf.int option * Ast.ctype * Ast.expression * bool =
      (* evalExpr should not be called with PT.EmptyExpr *)
    let
	  ...
	  case coreExpr
		...
		(case evalAstExpr e0
		   of SOME i => if (i = IntInf.fromInt 0) 
		                then evalAstExpr e2
				else evalAstExpr e1
		    | NONE    => NONE)
	     | Ast.Cast (ct,e) => 
	...
	in
	  if isSome opt andalso isSome opt' then
		...
		val i' = valOf opt'
	    in case binop
		 of Ast.Plus   => SOME (IntInf.+(i, i'))
		  | Ast.Minus  => SOME (IntInf.-(i, i'))
		  | Ast.Times  => SOME (IntInf.*(i, i'))
		  | Ast.Divide => SOME (IntInf.quot (i,i'))
		  | Ast.Mod    => SOME (IntInf.rem (i,i'))
		  | Ast.Gt     => SOME (if IntInf.>(i, i') then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Lt     => SOME (if IntInf.<(i, i') then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Gte    => SOME (if IntInf.>=(i, i') then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Lte    => SOME (if IntInf.<=(i, i') then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Eq     => SOME (if i = i' then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Neq    => SOME (if i <> i' then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.And    => SOME (if i<> IntInf.fromInt 0 andalso i'<> IntInf.fromInt 0 
					    then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Or     => SOME (if i<> IntInf.fromInt 0 orelse i'<> IntInf.fromInt 0 
					    then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.BitOr  => 
		      SOME (IfromWord (W.orb (ItoWord i, ItoWord i')))  
		  | Ast.BitXor => 
		      SOME (IfromWord (W.xorb (ItoWord i, ItoWord i')))  
		  | Ast.BitAnd => 
		      SOME (IfromWord (W.andb (ItoWord i, ItoWord i')))  
		  | Ast.Lshift => 
		      SOME (IfromWord (W.<< (ItoWord i, ItoWord i')))  
		  | Ast.Rshift =>
		      SOME (IfromWord (W.>> (ItoWord i, ItoWord i'))) 
	       | _      => NONE
	    end
	...
	in
	  if isSome opt then
	    let
	      val i  = valOf opt
	    in case unop
		 of Ast.Negate => SOME (IntInf.~i)
		  | Ast.Not    => SOME (if i = IntInf.fromInt 0 then IntInf.fromInt 1 else IntInf.fromInt 0)
		  | Ast.Uplus  => SOME i
		  | Ast.BitNot => SOME (IfromWord (W.notb (ItoWord i)))
		  | _      => NONE
	    end
	  else NONE
	end
